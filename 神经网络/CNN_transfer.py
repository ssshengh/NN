...
# 生成一个sample_size大小的，范围是range(len(sample_size))的随机列表，里面无重复
...
val_indices2 = np.random.permutation(range(val_size))  # 随机排列一个序列，或者数组。如果x是多维数组，则沿其第一个坐标轴的索引随机排列数组。
test_indices1 = range(val_size, len(test_dataset))  # （5000, 10000） range
...
# %%    可以随时从磁盘中读取，但是加载的时候，需要重新定义源网络的类
...
print('测试数据集上的分类准确度:{}'.format(right_rate))
# %%
# 随便从测试集中读入一张图片，并绘制出来
idx = 4
...
# %%
print(results['notransfer'][1], results['transfer_pretrained'][1], results['transfer_fixed'][1])
experiments = {}
